...
class OperationPartition:
    ...
    right_boundary_stage_start_times: dict[str, list[int]] | None = None
...
class PwCpSubproblemSpec:
    ...
    right_boundary_stage_start_times: dict[str, list[int]]
    is_last_batch: bool
...
class PwCpConstructor:
    ...
    def _build_subproblem_spec(
        ...
        assert partition.right_boundary_stage_start_times is not None, (
            "Right start times must be set"
        )
        ...
        return PwCpSubproblemSpec(
            ...
            right_boundary_stage_start_times=partition.right_boundary_stage_start_times,
    ...
    def _compute_right_boundary_profile(
        ...
        right_time_fixed_ops: tuple[OperationRef, ...],
        stage_2_job_2_p_dict: dict[str, dict[str, int]],
    ) -> tuple[StageBoundaryProfile, dict[str, list[int]]]:
        ...
        stage_2_start_times: dict[str, list[int]] = {
            stage_id: [] for stage_id in shifted.stages
        }
        right_boundary_profile: StageBoundaryProfile = {}
        for job_id, stage_id, mc_id in right_time_fixed_ops:
            stage_2_start_times[stage_id].append(start_map[job_id, stage_id, mc_id])
        for stage_id in shifted.stages:
            starts = [int(start) for start in sorted(stage_2_start_times[stage_id])]
            mc_cnt = len(shifted.machines_per_stage[stage_id])
            padded = starts[:mc_cnt]
            if len(padded) < mc_cnt:
                padded.extend([int(incumbent.makespan)] * (mc_cnt - len(padded)))
            right_boundary_profile[stage_id] = padded
        return right_boundary_profile, stage_2_start_times
    ...
    def _compute_boundary_deviation_hint_values(
        ...
        stage_2_completion_times: dict[str, list[int]] = {}
        end_time_map = incumbent.get_jik_2_end_time_map()

        for job_id, stage_id, machine_id in optimization_ops:
            stage_2_completion_times.setdefault(stage_id, []).append(
                end_time_map[job_id, stage_id, machine_id]
            )

        stage_max_deviations: list[int] = []
        hint_values: dict[str, int] = {}
        for stage_id, completion_times in stage_2_completion_times.items():
            sorted_completion_times = sorted(completion_times, reverse=True)
            mc_cnt = len(params.M_of[stage_id])
            padded_completion_times = completion_times[:]
            while len(padded_completion_times) < mc_cnt:
                padded_completion_times.append(0)
            while len(sorted_completion_times) < mc_cnt:
                sorted_completion_times.append(0)

            for k, c_value in enumerate(sorted_completion_times, start=1):
                hint_values[f"C_{stage_id}_{k}"] = c_value
                for idx, completion_value in enumerate(padded_completion_times):
                    hint_values[f"kth_completion_{stage_id}_{k}_ge_{idx}"] = int(
                        completion_value >= c_value
                    )
                    hint_values[f"kth_completion_{stage_id}_{k}_gt_{idx}"] = int(
                        completion_value > c_value
                    )

            aligned_boundary = list(reversed(right_boundary_profile[stage_id]))
            deviations = [
                completion_time - boundary_start
                for completion_time, boundary_start in zip(
                    sorted_completion_times, aligned_boundary, strict=False
                )
            ]
    ...
    def _build_operation_partition(
        ...
        - right_boundary_profile: stage boundary based on right-justified schedule
        """
        ...
        right_profile, right_start_times = self._compute_right_boundary_profile(
            incumbent,
            tuple(right_ops),
            stage_2_job_2_p_dict,
        )
        ...
        return OperationPartition(
            ...
            right_boundary_stage_start_times=right_start_times,
        )
    ...
    def _save_solution_dict(
        ...
        *,
        ...
        solution_dict = {
            ...
            "right_boundary_profile": spec.right_boundary_profile,
            "right_boundary_stage_start_times": spec.right_boundary_stage_start_times,
        }
