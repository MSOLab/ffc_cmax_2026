...
def test_draw_pw_cp_subproblem_progress_plots_creates_one_plot_per_subproblem(
    monkeypatch, tmp_path: Path
):
    log_path = tmp_path / "4-pw_cp_obj_log.yaml"
    dump_yaml(
        {
            "obj_value": {
                "name": "ObjVal after PW-CP batch",
                "data": {"1.0": 100},
                "notes": {"1.0": "batch=1"},
            },
            "obj_bound": {
                "name": "ObjVal before PW-CP batch",
                "data": {"1.0": 100},
                "notes": {"1.0": "batch=1"},
            },
            "pw_cp_metadata": {
    ...
    concurrent_painter._draw_pw_cp_subproblem_progress_plots(
        file_path=log_path,
        base_output_path=tmp_path / "4-pw_cp_progress_plot.png",
        encoding="utf-8",
    )
...
def test_draw_pw_cp_subproblem_progress_plots_skips_when_metadata_missing(
    monkeypatch, tmp_path: Path
):
